package com.youyu.oauth2.extensition.password;
...
public class PasswordAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        // 新建一个附加响应字段的 Map
        Map<String, Object> customAdditionalParameters = new LinkedHashMap<>();

        // 如果需要 ID Token（OIDC）
        if (idToken != null) {
            customAdditionalParameters.put(OidcParameterNames.ID_TOKEN, idToken.getTokenValue());
        }

        // 添加userInfo到附加数据
        Object principal = usernamePasswordAuthentication.getPrincipal();
        if (principal instanceof LoginUser userDetails) {
            customAdditionalParameters.put("userInfo", userDetails.getUser());
        }
